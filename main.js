...
function updateResults(){
  ...
  // Get common items for blue buttons only, excluding overlaps with all-selected items
  const commonItemsInBlue = getCommonItems(blueButtons).filter(item => !commonItemsInAll.includes(item));
 ...
 if(commonItemsInAll.length>0){
  ...
  // Add common items in blue buttons only
  if (commonItemsInBlue.length > 0) {
    const header = document.createElement('li');
    header.textContent = 'Common in blue buttons:';
    header.style.fontWeight = 'bold';
    itemList.appendChild(header);

    commonItemsInBlue.forEach(item => {
        const li = document.createElement('li');
        li.textContent = item;
        li.classList.add('blue-item'); // Apply blue text style
        itemList.appendChild(li);
    });
}
    ...
    console.log('Items to check for related items:', itemsToCheck); // Debug log
